<template>
  <div class="col-md-4 cardcontainer">
    <div class="card cardStyle">
      <img :src="`${item.Item.imageUrl}`" class="card-img-top" width="100%" />
      <div class="card-body px-0">
        <div class="cardBorder">
          <div class="d-flex flex-row justify-content-between mb-0 px-3">
            <h5 class="text-muted mt-1">PRICE</h5>
          ...
          <div class="d-flex flex-row justify-content-between px-3 pb-4">
            <div class="d-flex flex-column">
              <h6
                v-if="item.Item.status === `available`"
                class="text-muted font-weight-bold"
              >
                {{ item.Item.status }}
              </h6>
              <h6 v-else class="text-muted font-weight-bold">
                {{ item.Item.status }} for {{ item.Item.readyIn }} days
              </h6>
            </div>
          </div>
        </div>
